...
def cal_descriptors(row):  
    """
    calculate the descriptors for each image
    """
    features = []
...
def cal_cluster(cluster_num=500):  
    ...
    for f in features:
        bag_of_features += f
    clusters = kmeans(np.array(bag_of_features).astype(
        'float32'), 500, initialization="PLUSPLUS")  # kmean cluster
    return clusters


def cal_phow_features(row, clusters):
    """
    using the clustering centre to construct the phow feature vectors
    """
    img = cv.imread(row.path)
    gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)
    _, descriptors = phow(gray.astype(np.float32), step=4)
    # cal the distance to the each cluster centre
    dist = distance.cdist(clusters, descriptors, metric='euclidean')
...
def cal_path2phow_features(clusters):
    """
    before train the network, calculate phow for both training and testing image
    """
    ...
    if not os.path.exists('train_phow_feature.pkl'):
        ...
        train_list = pd.read_csv('train_list.csv')
        train_list['phow_feature'] = train_list.parallel_apply(
            cal_phow_features, axis=1, args=(clusters,))
    ...
    if not os.path.exists('test_phow_feature.pkl'):
        ...
        test_list['phow_feature'] = test_list.parallel_apply(
            cal_phow_features, axis=1, args=(clusters,))
    ...
    df = pd.concat([train_list[['path', 'phow_feature']],
                   test_list[['path', 'phow_feature']]], ignore_index=True)
