...
def preprocess():
    ...
    for f in train_path: #collect the path and label of train images
        ...
        for image in os.listdir('training/'+f):
            if  image =='.DS_Store' or image == '.ipynb_checkpoints':
                continue
            train_list = train_list.append({'path':'training/'+f+'/'+image,'class':f,'label':str(class2label[f])}, ignore_index=True) 
    ...
    for image in test_path: #collect the path of test images
        if image =='.DS_Store' or image == '.ipynb_checkpoints':
            continue
        test_list = test_list.append({'path':'testing/'+image,'predict_label':str(-1)}, ignore_index=True) 
